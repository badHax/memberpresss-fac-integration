...
{
    // static variables
    public static $settings_link = '/wp-admin/admin.php?page=fac-api';
    public static $payment_success_page = '/thank-you/';
    public static $paymentFailedPage = '/failed-payment';
    public static $mepr_api_create_sub_url = '/wp-json/mp/v1/subscriptions';
    public static $mepr_api_get_membership_url = '/wp-json/mp/v1/memberships/';
    public static $mepr_api_delete_sub_url = '/wp-json/mp/v1/subscriptions/';
    public static $mepr_api_get_subs_url = '/wp-json/mp/v1/subscriptions?page=1&per_page=10000';
    public static $mepr_api_transactions_url = '/wp-json/mp/v1/transactions';

    private static function get_domain($link)
    {
        $is_http = (empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'off') || $_SERVER['SERVER_PORT'] != 443;
        $protocol = $is_http ? 'http' : 'https';
        
        return $protocol . '://' . $_SERVER['SERVER_NAME'] . $link;
    }
	...
	//            echo "</h1>";
				...
				try {
					...
					FacApi::mepr_fail_page("Could not create subscription." . $ex->getMessage());
				}
    }
    ...
    {
        array_unshift($links, '<a href="'.get_domain(Self::settings_link). '/wp-admin/admin.php?page=fac-api">Settings</a>');
        return $links;
    }
    ...
    static function create_plugin_database_table()
    {
        ...
        if ($wpdb->get_var("show tables like '$wp_track_table'") != $wp_track_table) {
            ...
            $sql .= "  `customer_ref`  int(11)   NOT NULL, ";
...
//            self::write_log($wpdb->get_var("show tables like '$wp_track_table'"));
    ...
    static function db_create_recurring_txn($subId, $membership_id, $customer_reference, $amount, $pan, $cycles, $cycle_num, $count = 1)
    {
        ...
        $result = $wpdb->insert($wp_track_table, array(
                ...
                'customer_ref' => (int)$customer_reference,
                'txn_cycles' => $cycles,
                'txn_amount' => $amount,
    ...
    {
        wp_redirect(Self::get_domain(Self::$paymentSuccessPage));
        exit();
    }
    ...
    {
        $url = add_query_arg( 'err', urlencode_deep( $err), Self::get_domain(Self::$paymentFailedPage) );
        wp_redirect(Self::get_domain(Self::$paymentFailedPage).$err);
        exit();
    }
    ...
    static function mepr_create_sub($orderId, $price, $resp, $pan, $limit = 1, $limitType = "years", $limit_cycles = false)
    {
        ...
        $result = wp_remote_post(Self::get_domain(Self::mepr_api_create_sub_url), array(
            "body" => $data,
            "method" => "POST",
            "headers" => $headers
        ));
    ...
    static function mepr_delete_sub($id)
    {
        ...
        $result = wp_remote_post(Self::get_domain(Self::$mepr_api_delete_sub_url).''.$id, array(
            "method" => "DELETE",
            "headers" => $headers
        ));
        FacApi::write_log("============================================================");
    ...
    static function mepr_get_membership($id)
    {
        ...
        $result = wp_remote_get(Self::get_domain(Self::mepr_api_get_membership_url).''.$id, array(
            "headers" => $headers
        ));
        FacApi::write_log($result);
        if (wp_remote_retrieve_response_code($result) == 200) {
    ...
    static function mepr_get_subs()
    {
        ...
        $result = wp_remote_get(Self::get_domain(Self::mepr_api_get_subs_url), array(
            "method" => "GET",
            "headers" => $headers
        ));
        //FacApi::write_log($result);
                                            ...
                                            $send_welcome_email = false)
    {
        ...
        $result = wp_remote_post(Self::get_domain(Self::mepr_api_transactions_url), array(
            "body" => $data,
            "method" => "POST",
            "headers" => $headers
        ));
    ...
    static function display_fac_hosted_payment_form()
    {
        ?>
        <div id="payment_form">
			<form class = "mepr-signup-form mepr-form" action="javascript:void(0);" novalidate>
				<div class="mp-form-submit">
            		<label for="mepr_no_val" class="mepr-visuallyhidden">No val</label>
					<input type="text" id="mepr_no_val" name="mepr_no_val" class="mepr-form-input mepr-visuallyhidden mepr_no_val mepr-hidden" autocomplete="off">
      				<input type="button" class="mepr-submit" value="Continue" id="submit-trxn">
      				<img alt="Loading..." style="display: none;" title="Loading icon" data-src="https://cshrpteam.com/wp-admin/images/loading.gif" class="mepr-loading-gif lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="><noscript><img src="https://cshrpteam.com/wp-admin/images/loading.gif" alt="Loading..." style="display: none;" class="mepr-loading-gif" title="Loading icon" /></noscript>
    				</div>
			</form>
        </div>
        <script>
            window.addEventListener("load", function () {
               ...
               document.getElementById('submit-trxn').addEventListener('click', function(){
				   processPayment();
			   });
			   document.getElementById('submit-trxn').setAttribute('value','submit');
            });
